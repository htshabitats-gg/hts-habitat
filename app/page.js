...
const listings = [
  ...
  },
  {
    ...
    description:
      "Appartement de 45 m² au 2e étage du même immeuble, inspiré de l'Antiquité romaine revisitée dans une lecture contemporaine. Une chambre double et un canapé convertible pour un séjour original et mémorable.",
...
const faq = [
  {
    ...
    a: "Le check-in est possible à partir de 15h et le check-out avant 15h. L'arrivée est 100% autonome grâce à une boîte à clés sécurisée.",
  },
  {
  ...
  },
  {
    ...
    a: "Le Cocon Bohème peut accueillir jusqu'à 4 personnes avec ses 2 chambres. Rome Antique Moderne accueillera jusqu'à 3 personnes avec 1 chambre double et un canapé convertible.",
  },
  {
...
export default function HomePage() {
  ...
  const handleWaitlist = (e) => { e.preventDefault(); if (waitlistEmail) setWaitlistSent(true); };
  ...
  const ReservationCard = ({ href, title, badge, desc, cta }) => (
    <a href={href} target="_blank" rel="noreferrer"
      style={{ display:"flex", flexDirection:"column", gap:"1rem", background:"rgba(255,255,255,.1)", border:"2px solid rgba(255,255,255,.25)", borderRadius:6, padding:"2rem", textDecoration:"none", color:"white", transition:"background .2s, transform .2s, border-color .2s" }}
      onMouseEnter={e => { e.currentTarget.style.background="rgba(255,255,255,.17)"; e.currentTarget.style.transform="translateY(-3px)"; e.currentTarget.style.borderColor="rgba(255,255,255,.5)"; }}
      onMouseLeave={e => { e.currentTarget.style.background="rgba(255,255,255,.1)"; e.currentTarget.style.transform="translateY(0)"; e.currentTarget.style.borderColor="rgba(255,255,255,.25)"; }}
    >
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        <span style={{ fontFamily:"var(--serif)", fontSize:"1.4rem", fontWeight:600 }}>{title}</span>
        <span style={{ fontSize:".68rem", letterSpacing:".12em", textTransform:"uppercase", background:"var(--terra)", padding:".3rem .8rem", borderRadius:2 }}>{badge}</span>
      </div>
      <p style={{ fontSize:".85rem", color:"rgba(255,255,255,.65)", fontWeight:300, lineHeight:1.6, flex:1 }}>{desc}</p>
      <span style={{ fontSize:".75rem", letterSpacing:".1em", textTransform:"uppercase", borderBottom:"1px solid rgba(255,255,255,.35)", paddingBottom:".3rem", alignSelf:"flex-start" }}>{cta} →</span>
    </a>
  );
    ...
    <>
      <style>{`
        ...
        *:focus-visible { outline: 2px solid var(--terra); outline-offset: 3px; border-radius: 2px; }
        ...
        .hero-stats { position: relative; z-index: 3; display: flex; gap: 2.5rem; padding-top: 2.5rem; margin-top: 2.5rem; margin-bottom: 3rem; border-top: 1px solid rgba(255,255,255,.1); animation: fade-in-up .8s 1.1s ease both; flex-wrap: wrap; justify-content: center; }
        ...
        .marque-hero-title { font-family: var(--serif); font-size: clamp(2.5rem, 4vw, 4rem); font-weight: 300; color: white; line-height: 1.1; }
        ...
        .lodgify-wrap { margin-top: 1.5rem; padding: 1.5rem; background: rgba(255,255,255,.06); border: 1px solid rgba(255,255,255,.12); border-radius: 6px; }
        ...
        .form-info a { color: var(--terra); }
        .form-error { font-size: .72rem; color: #c0392b; margin-top: .3rem; }
        ...
        /* FOOTER */
        footer { background: var(--ink); color: rgba(255,255,255,.45); padding: 3rem 8vw; display: flex; justify-content: space-between; align-items: center; gap: 2rem; flex-wrap: wrap; }
                  ...
                  {l.gallery.length > 1 && (
                    <div className="card-thumbs">
                      {l.gallery.slice(1,4).map((src,j) => (
                        <div key={j} className="card-thumb" onClick={() => openLightbox(l.gallery, j+1)}><img src={src} alt="" /></div>
                      ))}
                    </div>
                  )}
                </div>
                <div className="card-body">
                  ...
                  <div className="card-footer">
                    {l.available ? (
                      ...
                      <div style={{ width:"100%" }}>
                        <p style={{ fontSize:".78rem", color:"var(--ink-soft)", marginBottom:".8rem", display:"flex", alignItems:"center", gap:".5rem" }}><Clock size={14} /> Ouverture imminente</p>
                        {waitlistSent ? (
      ...
      <section id="reservation" style={{ background:"var(--ink)", color:"white" }}>
        <div style={{ maxWidth:1300, margin:"0 auto" }}>
          ...
          <p className="section-sub" style={{ color:"rgba(255,255,255,.65)", maxWidth:480 }}>Réservez en toute sécurité sur Airbnb ou Booking, ou directement sans frais de plateforme.</p>

          {/* Airbnb + Booking en avant */}
          <div className="resa-grid" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"1.5rem", marginTop:"2.5rem" }}>
            <ReservationCard href={AIRBNB_LINK} title="Airbnb" badge="★ 4,9" desc="Réservez en toute sécurité avec la protection voyageur Airbnb. Paiement sécurisé, assistance 24h/24." cta="Voir l'annonce" />
            <ReservationCard href={BOOKING_LINK} title="Booking.com" badge="9,4 / 10" desc="Idéal si vous avez déjà un compte Booking. Annulation flexible, confirmation immédiate." cta="Voir sur Booking" />
          </div>
          ...
          <div style={{ marginTop:"1.5rem", padding:"1.5rem", background:"rgba(255,255,255,.05)", border:"1px solid rgba(255,255,255,.1)", borderRadius:4, display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:"1rem" }}>
            <div>
              <p style={{ fontSize:".78rem", letterSpacing:".1em", textTransform:"uppercase", color:"rgba(255,255,255,.45)", marginBottom:".3rem" }}>Réservation directe</p>
              <p style={{ fontSize:".85rem", color:"rgba(255,255,255,.55)", fontWeight:300 }}>Sans frais de plateforme — disponibilités en temps réel</p>
            </div>
            <div style={{ display:"flex", gap:"1rem", alignItems:"center" }}>
              <a href={LODGIFY_LINK} target="_blank" rel="noreferrer" style={{ background:"rgba(255,255,255,.12)", border:"1px solid rgba(255,255,255,.25)", color:"white", textDecoration:"none", padding:".65rem 1.4rem", borderRadius:3, fontSize:".75rem", letterSpacing:".1em", textTransform:"uppercase", fontWeight:500 }}>Réserver en direct</a>
              <a href={WHATSAPP_LINK} target="_blank" rel="noreferrer" style={{ color:"rgba(255,255,255,.45)", fontSize:".78rem", textDecoration:"none", letterSpacing:".06em" }}>WhatsApp →</a>
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="contact" style={{ background:"white", padding:"100px 8vw" }}>
        <div className="contact-grid" style={{ maxWidth:1300, margin:"0 auto", display:"grid", gridTemplateColumns:"1fr 1fr", gap:"80px", alignItems:"start" }}>
          <div className="fade-up">
            ...
            {[
              ...
              { icon:<MapPin size={15} />, label:"Adresse", value:"Narbonne, proche gare et centre-ville" },
            ].map(d => (
            ...
            <a href={WHATSAPP_LINK} target="_blank" rel="noreferrer" className="wa-btn"><MessageCircle size={18} /> Écrire sur WhatsApp</a>
          </div>
            ...
            {contactSent ? (
              ...
              <form className="contact-form" onSubmit={handleContact}>
                <div className="form-row">
                  <div className="form-group"><label htmlFor="prenom">Prénom</label><input id="prenom" type="text" placeholder="Votre prénom" autoComplete="given-name" value={contactForm.prenom} onChange={e => setContactForm({ ...contactForm, prenom: e.target.value })} /></div>
                  <div className="form-group"><label htmlFor="nom">Nom</label><input id="nom" type="text" placeholder="Votre nom" autoComplete="family-name" value={contactForm.nom} onChange={e => setContactForm({ ...contactForm, nom: e.target.value })} required /></div>
                </div>
                <div className="form-group"><label htmlFor="email">Email</label><input id="email" type="email" placeholder="votre@email.com" autoComplete="email" value={contactForm.email} onChange={e => setContactForm({ ...contactForm, email: e.target.value })} required /></div>
                <div className="form-group">
                  <label htmlFor="logement">Logement</label>
                  <select id="logement" value={contactForm.logement} onChange={e => setContactForm({ ...contactForm, logement: e.target.value })}>
                    <option>Cocon Bohème</option><option>Rome Antique Moderne</option><option>Je ne sais pas encore</option>
                  </select>
                </div>
                <div className="form-group"><label htmlFor="message">Message</label><textarea id="message" rows={4} placeholder="Votre question ou demande…" value={contactForm.message} onChange={e => setContactForm({ ...contactForm, message: e.target.value })} required /></div>
                <div className="form-consent">
                  <input id="rgpd-consent" type="checkbox" checked={consent} onChange={e => { setConsent(e.target.checked); if (e.target.checked) setConsentError(false); }} />
                  <label htmlFor="rgpd-consent">J'accepte que les informations renseignées soient utilisées uniquement pour répondre à ma demande. Je peux exercer mes droits à l'adresse <a href="mailto:greg.hosteins@gmail.com">greg.hosteins@gmail.com</a>.</label>
                </div>
                {consentError && <div className="form-error">Merci de cocher la case de consentement avant d'envoyer votre message.</div>}
                <div className="form-info">Les données transmises via ce formulaire sont utilisées uniquement pour répondre à votre demande. <a href="/mentions-legales">Mentions légales</a> · <a href="/politique-de-confidentialite">Politique de confidentialité</a>.</div>
